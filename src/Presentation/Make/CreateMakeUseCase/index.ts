...
export class CreateMakeUseCase {
  ...
  async execute(values: CreateMakeDto[], files: UploadedFile[]) {
    ...
    const makesValues = values.map((val) => {
      ...
      if (val.imageId) {
        makeImage = files.find(
          (file) =>
            JSON.parse(this._decodeUri.execute(file.name)).id === val.imageId
        );
      }
      return {
        ...val,
        ...
        makeLogo: makeImage
          ? this._fileHandler.getFileName(makeImage.mimetype.split("/")[1])
          : undefined,
      };
    });
    ...
    if (verifyMakes.length > 0) {
      throw new AppError(
        `Following cars already exist ${JSON.stringify(
          verifyMakes
        )} please try again`,
        400
      );
    ...
    const createMakes: CreateMakeDbDto[] = makesValues.map((x) => ({
      ...
      year_founded: x.yearFounded ? +x.yearFounded : undefined,
    }));
    await this._makeRepository.create(createMakes);

    for (const makeValue of makesValues) {
      if (makeValue.makeImage) {
        await this._fileHandler.postFiles(
          makeValue.makeImage.data,
          makeValue.makeLogo
        );
      }
    }
